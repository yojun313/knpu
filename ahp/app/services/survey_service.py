"""설문지 문항(groups) 자동 생성 + 계층 변경 시 버전 분류·응답 가지치기.
...
def generate_matrices(
    ...
    group_id는 parent_uuid를 그대로 쓴다 — 부모 하나에 그룹 하나가 항상 1:1이라
    ...
    만든다(group_id = "alt:<leaf_uuid>", is_alternative=True로 표시해서 결과
...
def diff_matrices(old_matrices: list[dict], new_matrices: list[dict]) -> dict:
    """이전/새 groups를 비교해 group_id별 변경 내역을 만든다.

    반환값의 added_children/removed_children은 이미 받은 응답 중 무엇을 버리고
    무엇을 살릴지 그대로 실행 가능한 형태다 — prune_answers()가 이 결과를 받아
    실제로 응답 문서를 정리한다.
    """
    old_by_id = {m["group_id"]: m for m in old_matrices}
    new_by_id = {m["group_id"]: m for m in new_matrices}

    diff = {
        "removed_group_ids": sorted(set(old_by_id) - set(new_by_id)),
        "changes": {},
    }
...
def prune_answers(answers: dict, diff: dict) -> tuple[dict, bool]:
    """응답 문서 하나(answers: {group_id: {pair_id: value}})에 diff를 적용해
    ...
    for mid, pairs in answers.items():
        if mid in diff["removed_group_ids"]:
            changed = True
            continue
