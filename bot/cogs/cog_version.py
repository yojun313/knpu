...
class VersionBoardPoller(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        # 기존 bot.db를 쓰는 구조
        self.collection = self.bot.db["versions-board"]
        self.collection2 = self.bot.db["auth_config"]

        # 봇 실행 시 폴링 태스크 시작
        self.polling_task = self.bot.loop.create_task(
            self.start_polling()
        )
        ...
        try:
            ...
            async for document in cursor:
                ...
                if notified:
                    ...
                    update_log_channel = self.collection2.find_one({"guild_id": document.get("guild_id")}).get("update_log_channel")
                    ...
                    if update_log_channel:
                        ...
                        print(f"[알림 실패] 길드 ID {document.get('guild_id')} 의 업데이트 로그 채널이 설정되지 않음")
    ...
    @discord.app_commands.command(
        name="업데이트로그채널",
        description="업데이트 로그 채널 설정"
    )
    @discord.app_commands.describe(
        channel="업데이트 로그를 보낼 채널"
    )
