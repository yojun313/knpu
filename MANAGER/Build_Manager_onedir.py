...
def build_exe_from_spec(spec_file, output_directory, version):
    ...
    try:
        # Run pyinstaller with the new spec file and --onedir option
        subprocess.run([
            'pyinstaller',
            '--onedir',  # Add onedir option here
            '--distpath', output_directory,
            '--workpath', os.path.join(output_directory, 'build'),
            new_spec_file
        ])
        print(f"Finished building {exe_name}.exe in onedir mode.")
    finally:
        # Clean up the new spec file and build directory
