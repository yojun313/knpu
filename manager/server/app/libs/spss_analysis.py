"""크롤링 데이터 위에서 SPSS의 주요 분석 메뉴(기술통계·빈도분석·교차분석/카이제곱·
평균비교(t-검정/분산분석)·상관분석·회귀분석·요인분석(PCA)·신뢰도분석·군집분석)를
자동으로 수행하는 범용 통계 스위트.

statistics_analysis.py의 10개 분석 함수는 카테고리/플랫폼별로 정해진 열 이름을
전제하지만, 이 모듈은 그 반대로 결과 데이터프레임(각 분석 함수가 데이터에 덧붙인
파생 열 포함)에서 수치형/범주형으로 쓸만한 열을 스스로 찾아 검정을 돌린다. 그래서
category/platform과 무관하게 항상 마지막에 한 번 더 실행된다.

각 분석은 서로 독립적이라 하나가 실패해도(표본 부족, 상수 열 등) 나머지는 정상적으로
표를 만들어야 하므로 run()의 safe()로 개별 감싼다."""
