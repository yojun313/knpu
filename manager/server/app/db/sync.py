...
def sync_manager_databases():
    ...
    for target in targets:
        ...
        for coll_name in src_db.list_collection_names():
            src_db[coll_name].aggregate(
                [{"$out": {"db": target_db_name, "coll": coll_name}}]
            )
            ...
            for index_name, index_info in indexes.items():
                ...
                keys = index_info["key"]
                options = {
                    k: v for k, v in index_info.items() if k not in ["v", "key", "ns"]
                }
...
def migrate_kst(collection_name):
    ...
    for doc in all_docs:
        ...
        coll.update_one(
            {"_id": doc["_id"]}, {"$set": {"datetime_kst": datetime_kst_str}}
        )
