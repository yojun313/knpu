...
def sendPushOver(msg, user_key="uvz7oczixno7daxvgxmq65g2gbnsd5", image_path=False):
    ...
    for app_key in app_key_list:
        try:
            ...
            # 메시지 내용
            message = {"token": app_key, "user": user_key, "message": msg}
            # Pushover에 요청을 보냄
            if image_path == False:
                ...
                response = requests.post(
                    ...
                    data=message,
                    files={
                        "attachment": ("image.png", open(image_path, "rb"), "image/png")
                    },
                )
            break
        except:
...
def build_exe_from_spec(spec_file, output_directory, version):
    ...
    new_spec_file = os.path.join(output_directory, f"MANAGER_{version}.spec")
    create_spec_file(spec_file, new_spec_file, exe_name)

    try:
        # Run pyinstaller with the new spec file
        venv_python = os.path.join(
            "C:/GitHub/knpu/venv", "Scripts", "python.exe"
        )  # 가상환경 안의 python
...
if __name__ == "__main__":
    ...
    while True:
        ...
        console.print(
            Panel.fit(
                f"[bold magenta]Inno Setup 버전 정보 업데이트", title="setup.iss 처리"
            )
        )
