...
def downloadProgram(parent, newVersionName, is_update=True):
    ...
    if is_update:
        ...
        download_url = f"https://manager.knpu.re.kr/api/download/file/MANAGER_{newVersionName}_update.exe"
        msg = "업데이트 다운로드"
    else:
        downloadFile_path = os.path.join(temp_dir, f"MANAGER_{newVersionName}.exe")
        download_url = (
            f"https://manager.knpu.re.kr/api/download/file/MANAGER_{newVersionName}.exe"
        )
        msg = "전체 패키지 다운로드"
