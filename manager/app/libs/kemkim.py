...
class KimKem:
    ...
    def make_kimkem(self):
        try:
            ...
            print("\n토큰 데이터 분할 중... ")
            printStatus(self.main, '토큰 데이터 분할 중...')
            ...
            if keyword_list == []:
                ...
                printStatus(self.main, "키워드 없음 종료")
                return 0

            self.write_status("TF/DF 계산 중...")
            print("\nTF/DF 계산 중...")
            printStatus(self.main, 'TF/DF 계산 중...')
            ...
            print("\n추적 데이터 DOV/DOD 계산 중...")
            printStatus(self.main, '추적 데이터 DOV/DOD 계산 중...')
            ...
            if self.filter_option == True:
                if get_setting('ProcessConsole') == 'default':
                    iterator = tqdm(self.period_list, desc="기간별 추적 KEMKIM 데이터",
                ...
                for index, period in iterator:
                    ...
                    result_folder = os.path.join(
                        self.trace_result_folder, period)

                    self.write_status(f"{period} KEMKIM 증가율 계산 중...")
                    printStatus(self.main, f"{period} KEMKIM 증가율 계산 중...")
                    ...
                    if self.trace_standard == 'prevyear':
                        avg_DoV_increase_rate, avg_DoD_increase_rate, avg_term_frequency, avg_doc_frequency = self._calculate_averages(
                            keyword_list, trace_DoV_dict, trace_DoD_dict, tf_counts, df_counts, self.period_list[index-1], period)
                    else:
                        avg_DoV_increase_rate, avg_DoD_increase_rate, avg_term_frequency, avg_doc_frequency = self._calculate_averages(
                            keyword_list, trace_DoV_dict, trace_DoD_dict, tf_counts, df_counts, self.period_list[0], period)

                    self.write_status(f"{period} KEMKIM 신호 분석 및 그래프 생성 중...")
                    printStatus(
                        self.main, f"{period} KEMKIM 신호 분석 및 그래프 생성 중...")

                    # Step 8: 신호 분석 및 그래프 생성
                    DoV_signal_record[period], DoD_signal_record[period], DoV_coordinates_record[period], DoD_coordinates_record[period] = self._analyze_signals(
                        avg_DoV_increase_rate, avg_DoD_increase_rate, avg_term_frequency, avg_doc_frequency, os.path.join(result_folder, 'Graph'))
                    Final_signal_record[period] = self._save_final_signals(
                        DoV_signal_record[period], DoD_signal_record[period], os.path.join(result_folder, 'Signal'))

                # Trace 데이터에서 키워드별로 Signal 변화를 추적
                self.write_status("키워드 추적 데이터 생성 및 필터링 중...")
                print("\n키워드 추적 데이터 생성 및 필터링 중...")
                printStatus(self.main, '키워드 추적 데이터 생성 및 필터링 중...')
                ...
                if self.ani_option == True:
                    ...
                    print("\n키워드 추적 그래프 생성 중...")
                    printStatus(self.main, '키워드 추적 그래프 생성 중...')
                    ...
                    print("\n키워드 추적 애니메이션 생성 중...")
                    printStatus(self.main, '키워드 추적 애니메이션 생성 중...')
            ...
            print("\n[ 최종 KEM KIM 계산 ]\n")
            printStatus(self.main, '최종 KEM KIM 생성 중...')
    ...
    def cal_tf(self, keyword_list, period_divided_dic_merged):
        ...
        if get_setting('ProcessConsole') == 'default':
            iterator = tqdm(period_divided_dic_merged.items(
            ), desc="TF ", file=sys.stdout, bar_format="{l_bar}{bar}|", ascii=' =')
        else:
    ...
    def cal_df(self, keyword_list, period_divided_dic):
        ...
        if get_setting('ProcessConsole') == 'default':
            iterator = tqdm(period_divided_dic, desc="DF ",
                            file=sys.stdout, bar_format="{l_bar}{bar}|", ascii=' =')
        else:
    ...
    def cal_DoV(self, keyword_list, period_divided_dic, tf_counts, trace=True):
        DoV_dict = {}
        if get_setting('ProcessConsole') == 'default':
            iterator = tqdm(period_divided_dic, desc="DOV ",
                            file=sys.stdout, bar_format="{l_bar}{bar}|", ascii=' =')
        else:
            iterator = period_divided_dic
        for period in iterator:
