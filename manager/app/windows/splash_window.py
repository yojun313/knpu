...
class AboutDialog(BaseDialog):
    def __init__(self, version, theme="light", parent=None):
        ...
        self.bg_color = (
            QColor("#2b2b2b") if self.theme == "dark" else QColor(255, 255, 255)
        )
    ...
    def _build_ui(self):
        ...
        title_label = QLabel("MANAGER")
        title_label.setStyleSheet(
            f"font-size: 24px; font-family: 'Tahoma'; color: {self.text_color};"
        )
        text_layout.addWidget(title_label, alignment=Qt.AlignmentFlag.AlignLeft)

        version_label = QLabel(f"Version {self.version}")
        version_label.setStyleSheet(
            f"font-size: 18px; font-family: 'Tahoma'; color: {self.text_color};"
        )
        text_layout.addWidget(version_label, alignment=Qt.AlignmentFlag.AlignLeft)

        desc_label = QLabel(
            "MANAGER는 KNPU PAILAB에서 개발한 빅데이터 분석 및 관리 프로그램입니다."
        )
        desc_label.setWordWrap(True)
        desc_label.setStyleSheet(
            f"font-size: 14px; font-family: 'Tahoma'; color: {self.gray_color};"
        )
