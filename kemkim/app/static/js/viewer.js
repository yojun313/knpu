(function () {
  ...
  var ADMIN_ALL_KEY = 'kv_admin_all_projects';
  var isAdmin = false;
  ...
  var railFolders = [];
  var COLLAPSED_FOLDERS_KEY = 'kv_collapsed_folders';
  ...
  function buildProjectItem(p, interactive) {
    ...
    item.title = p.name;
    var s = p.summary || {};
    var sigMeta = s.strong_signal != null ? '핵심 신호어 ' + s.strong_signal + '개' : '';
    var meta = p.owner_name ? [p.owner_name, sigMeta].filter(Boolean).join(' · ') : sigMeta;
  ...
  function onAnalyzeFileSelected(file) {
    ...
    uploadWithProgress('/api/projects/analyze/upload', fd, function (pct) {
      ...
      if (res.suggested_start_date) document.getElementById('optStartDate').value = res.suggested_start_date;
      if (res.suggested_end_date) document.getElementById('optEndDate').value = res.suggested_end_date;
  ...
  function startCrawlAnalyze() {
    ...
    var statusEl = document.getElementById('crawlStatus');
    var sd = document.getElementById('crawlOptStartDate').value;
    var ed = document.getElementById('crawlOptEndDate').value;
    if (!sd || !ed) { statusEl.textContent = '분석 시작일/종료일을 선택해주세요.'; return; }
    var splitOption = document.getElementById('crawlOptSplit').value;
    var option = {
      startdate: sd.replace(/-/g, ''),
      enddate: ed.replace(/-/g, ''),
      period: document.getElementById('crawlOptPeriod').value,
      weight: parseFloat(document.getElementById('crawlOptWeight').value) || 0.1,
      topword: parseInt(document.getElementById('crawlOptTopword').value, 10) || 500,
      graph_wordcnt: parseInt(document.getElementById('crawlOptGraphWordcnt').value, 10) || 10,
      split_option: splitOption,
      split_custom: splitOption.indexOf('직접') === 0 ? String(parseInt(document.getElementById('crawlOptSplitCustom').value, 10) || 10) : null,
      trace_standard: document.getElementById('crawlOptTraceStandard').value,
      filter_option: document.getElementById('crawlOptFilter').checked,
      ani_option: document.getElementById('crawlOptAni').checked,
    };
  ...
  function exportPlotPng() {
    try {
      ...
      img.onload = function () {
        ...
        ctx.fillStyle = isDarkMode() ? '#212121' : '#ecf0f1';
  ...
  function bindEvents() {
    ...
    window.addEventListener('resize', function () { closeRailCtxMenu(); closeMoveFolderMenu(); onPlotResize(); });
    ...
    document.getElementById('crawlOptSplit').addEventListener('change', function () {
      document.getElementById('crawlSplitCustomField').hidden = this.value.indexOf('직접') !== 0;
    });
