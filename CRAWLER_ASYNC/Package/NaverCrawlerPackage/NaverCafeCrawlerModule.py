...
class NaverCafeCrawler(CrawlerModule):
    ...
    async def _cafeIDExtractor(self, cafeURL):
    
        response = await self.asyncRequester(cafeURL)
    ...
    async def articleCollector(self, cafeURL):
        ...
        try:
            ...
            cafeID = await self._cafeIDExtractor(cafeURL)
            ...
            response = await self.asyncRequester(api_url)
    ...
    async def replyCollector(self, cafeURL):
        if isinstance(cafeURL, str) == False or self._cafeURLChecker(cafeURL) == False:
    ...
    async def asyncSingleCollector(self, cafeURL, option):
        ...
        async with semaphore:
            ...
            replyData = await self.replyCollector(cafeURL)
    ...
    async def asyncMultiCollector(self, urlList, option):
        ...
        for cafeURL in urlList:
            tasks.append(self.asyncSingleCollector(cafeURL, option))
