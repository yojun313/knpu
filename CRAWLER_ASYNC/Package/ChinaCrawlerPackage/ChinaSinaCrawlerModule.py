...
class ChinaSinaCrawler(CrawlerModule):
    ...
    async def articleCollector(self, newsURL):
        ...
        try:
            main_page = await self.asyncRequester(newsURL)
        ...
        try:
            ...
            while True:
                ...
                if newsURL_type == 1:
                    ...
                    main_page = await self.asyncRequester(api_url)
                ...
                # https://news.sina.cn/sh/2023-01-29/detail-imycwefp0361917.d.html
                    ...
                    main_page = await self.asyncRequester(default_url, headers=headers, params=params)
                try:
