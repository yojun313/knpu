...
class data_process:
    ...
    def option_1(self):
        ...
        if "article" in self.file_name:
            self.csv_data['article date'] = pd.to_datetime(self.csv_data['News Date'].str.split().str[0], format='%Y.%m.%d.', errors='coerce')
            self.csv_data['year'] = self.csv_data['News Date'].dt.year
            self.csv_data['month'] = self.csv_data['News Date'].dt.month
            self.csv_data['year_month'] = self.csv_data['News Date'].dt.to_period('M')
            self.csv_data['week'] = self.csv_data['News Date'].dt.to_period('W')
            
        # 댓글
        elif "reply" in self.file_name:
            self.csv_data['reply date'] = pd.to_datetime(self.csv_data['Reply Date'], errors='coerce')
        ...
        # 대댓글
            ...
            self.csv_data['rereply date'] = pd.to_datetime(self.csv_data['Rereply Date'], errors='coerce')
