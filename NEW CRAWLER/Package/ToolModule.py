...
TOOLMODULE_PATH = os.path.dirname(os.path.abspath(__file__))  # ToolModule.py의 디렉토리
BIGMACLAB_PATH = os.path.abspath(os.path.join(TOOLMODULE_PATH, '../../'))  # '프로젝트' 디렉토리 기준으로 상위 디렉토리 이동
...
class ToolModule:
    def __init__(self):
        ...
        if getattr(sys, 'frozen', False):
            ...
            self.base_path = os.path.abspath(os.path.join(TOOLMODULE_PATH, '../..', 'CRAWLER_0922'))
            
    # Set folder path depending on the computer
    def pathFinder(self, name=''):
        crawler_folder_path = self.base_path
    ...
    def send_pushOver(self, msg, user_key):
        app_key_list  = ["app_key_1", "app_key_2"]
...
if __name__ == '__main__':
    ...
    mySQL_obj = pymysql.connect(host='localhost', user='root', password='0000', database = 'database')
    with mySQL_obj.cursor() as cursor:
        create_table_query = """
