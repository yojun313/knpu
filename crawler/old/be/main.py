...
class Crawler(CrawlerModule):
    ...
    def DBinfoRecorder(self, endoption=False, error=False):
        ...
        if error == True:
            requests.put(
                f"{self.api_url}/crawls/{self.dbUid}/error", headers=self.api_headers
            ).json()

        elif endoption == True:
            del self.IntegratedDB["UrlCnt"]
            requests.put(
                f"{self.api_url}/crawls/{self.dbUid}/end", headers=self.api_headers
            ).json()
            with open(
                os.path.join(self.crawllog_path, self.DBname + "_log.txt"), "r"
            ) as log:
                log_content = log.read()

            json = {"uid": self.dbUid, "content": log_content}
            requests.post(
                f"{self.api_url}/crawls/update/log", json=json, headers=self.api_headers
            ).json()
    ...
    def DBMaker(self, DBtype):
        ...
        res = requests.post(
            self.api_url + "/crawls/add", json=json, headers=self.api_headers
        ).json()
    ...
    def ReturnChecker(self, value):
        if isinstance(value, dict) == True:
            ...
            if first_key == "Error Code":
                ...
                log = open(
                    os.path.join(self.crawllog_path, self.DBname + "_log.txt"), "a"
                )
                msg = (
    ...
    def FinalOperator(self):
        try:
            ...
            res = requests.put(
                f"{self.api_url}/crawls/{self.dbUid}/count",
                json=self.IntegratedDB,
                headers=self.api_headers,
            ).json()

            parquet_files = [
                f for f in os.listdir(self.DBpath) if f.endswith(".parquet")
            ]
            ...
            starttime = datetime.fromtimestamp(self.startTime).strftime(
                "%Y-%m-%d %H:%M"
            )
            ...
            for dayCount in range(self.date_range + 1):
                try:
                    ...
                    urlList_returnData = NaverNewsCrawler_obj.urlCollector(
                        keyword=self.keyword,
                        startDate=self.currentDate_str,
                        endDate=self.currentDate_str,
                    )
                    ...
                    for returnData in FullreturnData:
                        ...
                        if (
                            self.ReturnChecker(article_returnData) == True
                            and article_returnData["articleData"] != []
                        ):
                            articleStatus = True
                        else:
                            continue
                        ...
                        if option == 3:
                            self.addToCSV(
                                tableName=self.articleDB,
                                data_list=[article_returnData["articleData"] + [0]],
                            ...
                            if self.ReturnChecker(replyList_returnData) == True:
                                ...
                                if replyList_returnData["replyList"] != []:
                                    data_list = [
                                        sublist + [article_returnData["articleData"][5]]
                                        for sublist in replyList_returnData["replyList"]
                                    ]
                                    self.addToCSV(
                                        tableName=self.replyDB,
                                        data_list=data_list,
                            ...
                            if option == 2:
                                ...
                                if (
                                    self.ReturnChecker(rereplyList_returnData) == True
                                    and rereplyList_returnData["rereplyList"] != []
                                ):
                                    data_list = [
                                        sublist + [article_returnData["articleData"][5]]
                                        for sublist in rereplyList_returnData[
                                            "rereplyList"
                                        ]
                                    ]
                                    self.addToCSV(
                                        tableName=self.rereplyDB,
                                        data_list=data_list,
                                        columns=rereply_column,
                                    )
                ...
                try:
                    ...
                    for returnData in FullreturnData:
                        ...
                        if option == 2:
                            replyList_returnData = returnData["replyData"]
                            if (
                                self.ReturnChecker(replyList_returnData) == True
                                and replyList_returnData["replyList"] != []
                            ):
                                data_list = [
                                    sublist + [article_returnData["articleData"][3]]
                                    for sublist in replyList_returnData["replyList"]
                                ]
                                self.addToCSV(
                                    tableName=self.replyDB,
        ...
        while self.running == True:
            for dayCount in range(self.date_range + 1):
                try:
                    self.currentDate_str = self.currentDate.strftime("%Y%m%d")
                    percent = str(round(((dayCount + 1) / self.date_range) * 100, 1))
                    YouTubeCrawler_obj.setPrintData(
                    ...
                    for returnData in FullreturnData:
                        ...
                        if self.ReturnChecker(replyList_returnData) == True:
                            if replyList_returnData["replyList"] != []:
                                data_list = [
                                    sublist + [article_returnData["articleData"][4]]
                                    for sublist in replyList_returnData["replyList"]
                                ]
                                self.addToCSV(
                                    tableName=self.replyDB,
            ...
            for DateRange in DateRangeList:
                try:
                    ...
                    ChinaSinaCrawler_obj.setPrintData(
                        f"{currentDate_str_start.strftime('%Y.%m.%d')} ~ {currentDate_str_end.strftime('%Y.%m.%d')}",
                        percent,
                        self.weboption,
                    )
                    ...
                    for returnData in FullreturnData:
                        ...
                        if option == 2:
                            ...
                            if (
                                ...
                                data_list = [
                                    sublist + [article_returnData["articleData"][2]]
                                    for sublist in replyList_returnData["replyList"]
                                ]
                                self.addToCSV(
                                    tableName=self.replyDB,
...
def manualTokenizer():
    ...
    DBpath = (
        "/mnt/ssd/bigmaclab/crawldata/navernews_보호관찰_20200101_20250331_0526_2122"
    )
