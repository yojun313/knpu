...
class NaverNewsCrawler:
    ...
    def fromResume(cls, DBuid, endDate=None):
        ...
        obj.DBname = renameForResume(
            DBuid, "navernews", obj.keyword, doc["startDate"], obj.endDate, obj.DBname
        )

        obj.articleDB = obj.DBname + "_article"
        obj.statisticsDB = obj.DBname + "_statistics"
        ...
        if obj.option in [1, 2, 4]:
            tables.append(obj.statisticsDB)
            tables.append(obj.replyDB)
            if obj.option == 2:
                tables.append(obj.rereplyDB)
    ...
    def collectUrl(self, keyword, startDate, endDate):
        try:
            startDate_formed = (
                datetime.strptime(str(startDate), "%Y%m%d").date().strftime("%Y.%m.%d")
            )
            endDate_formed = (
                datetime.strptime(str(endDate), "%Y%m%d").date().strftime("%Y.%m.%d")
            )
            ...
            # API URL 생성
            api_url = (
                f"https://s.search.naver.com/p/newssearch/3/api/tab/more?{query_string}"
            )
    ...
    async def collectArticle(self, session, newsURL):
        try:
            ...
            try:
                article_press = (
                    str(bs.find("img")).split()[1][4:].replace('"', "")
                )  # article_press
                article_type = bs.find(
                    "em", class_="media_end_categorize_item"
                ).text  # article_type
                article_title = bs.find(
                    "div", class_="media_end_head_title"
                ).text.replace("\n", " ")  # article_title
                article_date = bs.find(
                    "span",
            ...
            for i in range(len(nickname_list)):
                ...
                if text_list[i] != "":
                    targetlist = [
                        ...
                        str(nickname_list[i]),
                        datetime.strptime(
                            replyDate_list[i], "%Y-%m-%dT%H:%M:%S%z"
                        ).strftime("%Y-%m-%d"),
                        str(
                            text_list[i]
                            .replace("\n", " ")
                    ...
                    if username:
                        ...
                        if add_data:
                            targetlist[1] = (
                                f"{targetlist[1]}_{add_data[0]}_{add_data[1]}"
                            )
    ...
    async def collectReply(self, session, newsURL, parentCommentNum_list):
        try:
            ...
            returnData = {"rereplyList": rereplyList, "rereplyCnt": len(rereplyList)}
            ...
            for i in range(len(parentCommentNum_list)):
                try:
                    ...
                    target_url = (
                        base_url.format(oid, aid, 100, 1, "reply")
                        + "&parentCommentNo="
                    ...
                    r_like_list.extend(sympathy_counts)
                    r_bad_list.extend(antipathy_counts)
                    parentReplynum_list.extend(
                        [parentCommentNum_list[i]] * len(masked_user_ids)
                    )
                ...
                if text_list[i] != "":
                    rereplyList.append(
                        [
                            ...
                            str(nickname_list[i]),
                            datetime.strptime(
                                rereplyDate_list[i], "%Y-%m-%dT%H:%M:%S%z"
                            ).strftime("%Y-%m-%d"),
                            str(
                                text_list[i]
                                .replace("\n", " ")
