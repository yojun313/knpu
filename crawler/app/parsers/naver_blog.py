...
class NaverBlogCrawler:
    ...
    def fromResume(cls, DBuid, endDate=None):
        ...
        obj.DBname = renameForResume(
            DBuid, "naverblog", obj.keyword, doc["startDate"], obj.endDate, obj.DBname
        )
    ...
    async def collectArticle(self, session, blogURL):
        try:
            ...
            url = "https://blog.naver.com/PostView.naver?blogId={}&logNo={}&redirect=Dlog&widgetTypeCall=true&noTrackingCode=true&directAccess=false".format(
                blogID, logNo
            )
            headers = {
            ...
            try:
                article = "".join(
                    [
                        i.text.replace("\n", "").replace("\t", "").replace("\u200b", "")
                        for i in bs.select("div[class = 'se-module se-module-text']")
                    ]
                )
                date = "".join(
                    [i.text for i in bs.select("span[class = 'se_publishDate pcol2']")]
                )
    ...
    async def collectCmt(self, session, blogURL, username=False):
        try:
            ...
            url = "https://blog.naver.com/PostView.naver?blogId={}&logNo={}&redirect=Dlog&widgetTypeCall=false&directAccess=false".format(
                blogID, logNo
            )
            ...
            while True:
                ...
                script_tag = bs.find(
                    "script", string=re.compile(r"var\s+blogNo\s*=\s*\'(\d+)\'")
                )  # type: ignore[arg-type]
                match = re.search(
                    r"var\s+blogNo\s*=\s*\'(\d+)\'",
                    script_tag.text if script_tag else "",
                )
                if match:
            ...
            for c in all_comments:
                ...
                targetlist = [
                    str(reply_idx),
                    str(masked_user_id),
                    datetime.strptime(mod_time, "%Y-%m-%dT%H:%M:%S%z").strftime(
                        "%Y-%m-%d"
                    ),
                    contents.replace("\n", " ")
                    .replace("\r", " ")
