...
def notifyRequester(requester, email, title, text):
    ...
    디스코드 인증(discord_id 연동)이 되어있으면 크롤러 상태 알림 채널(crawler_status)에
    올리면서 요청자를 멘션한다. 봇이 실제로 채널 전송에 실패한 경우에만
    이메일로 폴백해서 보낸다(bot/cogs/cog_notifications.py의 _send_one 참고).
