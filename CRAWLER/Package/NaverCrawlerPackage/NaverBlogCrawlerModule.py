...
class NaverBlogCrawler(CrawlerModule):
    ...
    async def articleCollector(self, blogURL):
        trynum = 1
        ...
        try:
            while True:
                ...
                response = await self.asyncRequester(url, headers)
    ...
    async def replyCollector(self, blogURL):
        if isinstance(blogURL, str) == False or self._blogURLChecker(blogURL) == False:
        ...
        try:
            ...
            while True:
                try:
                    response   = await self.asyncRequester(url)
                    soup       = BeautifulSoup(response, "html.parser")
    ...
    async def asyncMultiCollector(self, urlList, option):
        ...
        for blogURL in urlList:
            tasks.append(self.asyncSingleCollector(blogURL, option))
