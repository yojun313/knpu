...
class NaverCafeCrawler:
    ...
    def escape_content_html(self, json_str):
        ...
        if match:
            ...
            escaped = content_html.replace('\\', '\\\\').replace('"', '\\"').replace('\n', '\\n').replace('\r', '\\r')
                    ...
                    if self.option == 1:
                        ...
                        try:
                            ...
                            if replies:
                                ...
                                addToCSV(self.DBPath, self.replyDB, processed_replies, navercafe_reply_column)

                        except Exception as e:
                            logger.info(f"Error occurred while processing cafe comment data for {cafeUrl}: {e}")
