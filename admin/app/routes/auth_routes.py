...
async def handle_login(request: Request, name: str = Form(...)):
    ...
    if not email:
        return templates.TemplateResponse(
            request=request,
            name="login.html",
            context={"error": "관리자 권한이 없거나 등록되지 않은 이름입니다."},
        )
...
async def verify_page(request: Request, name: str):
    return templates.TemplateResponse(
        request=request, name="verify.html", context={"name": name}
    )


@router.post("/verify")
async def handle_verify(
    response: Response, name: str = Form(...), code: str = Form(...)
):
    session_id = verify_login(name, code)
    if not session_id:
        return RedirectResponse(
            url=f"/verify?name={name}&error=invalid", status_code=302
        )
